...
class CosHead(nn.Module):
    ...
    def forward(self, input):
        return F.linear(F.normalize(input), F.normalize(self.weight, dim=1)).clamp(-1. + self.eps, 1. - self.eps) 
...
class ArcFace(nn.Module):
    """Implement of large margin arc distance: :
        Args:
            ...
            cos(theta + m)
        """
    def __init__(self, in_features, out_features,m=0.50, s=64, **kwargs):
...
class CosFace(nn.Module):
    ...
    def __init__(self, in_features, out_features, m=0.35, s=64.0, **kwargs):
...
class MagFace(nn.Module):
    ...
    def __init__(self, in_features, out_features, l_m=0.45, s=64.0, l_a=10, u_a=110, u_m=0.8, lambda_g=35, nonw=False):
    ...
    def forward(self, input, label):
        if self.nonw:
            cos_t = input.clamp(-1. + self.eps, 1. - self.eps) 
        else:
...
class ElasticArcFace(nn.Module):
    ...
    def __init__(self, in_features, out_features, m=0.50, s=64.0, std=0.0125,plus=False, nonw=False):
...
class ElasticCosFace(nn.Module):
    def __init__(self, in_features, out_features, m=0.35, s=64.0, std=0.0125, plus=False, nonw=False):
...
class AdaFace(nn.Module):
    def __init__(self, in_features, out_features, m=0.4, s=64., h=0.333, t_alpha=0.99, nonw=False):
